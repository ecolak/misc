...
import self.ec.btcbots.db.PersistenceManager;
...
public class GenericDao<T> {
  ...
  public long count(Criteria criteria) {
    ...
    try {
      ...
      for (Criteria.Condition condition : criteria.getCriteria()) {
        ...
        qb.append(condition.getColumnName()).append(" ")
            .append(condition.getOperator().getSymbol()).append(" :")
  ...
  public ResultList<T> query(Criteria criteria) {
    ...
    try {
      ...
      final TypedQuery<T> query =
          criteria.isPaged() ? getQuery(em, qb.toString(), criteria.getPage(),
              criteria.getPageSize()) : getQuery(em, qb.toString(), 0, 0);
  ...
  public int deleteBy(final String columnName, final Object columnValue) {
    ...
    try {
      ...
      transaction.begin();
      Query query =
          em.createQuery("delete from " + entityName + " where " + columnName + " = :" + columnName);
  ...
  private TypedQuery<T> getQuery(final EntityManager em, final String query, final int page,
      final int pageSize) {
