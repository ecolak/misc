...
public class CoinbaseRestClient {
	...
	private static TransactionResponse simulateBuyOrSell(float quantity, float price, String path) {
		...
		response.transfer.fees = new Fee[]{
				new Fee(new Price(String.valueOf(coinbaseFee) ,USD), 
				new Price(String.valueOf(bankFee), USD))};
	...
	private static TransactionResponse buyOrSell(String apiKey, float quantity, float price, String path) {
		return client.target(BASE_URL)
				.path(path)
				.request(MediaType.APPLICATION_JSON)
